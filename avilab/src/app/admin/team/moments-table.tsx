...
export function MomentsTable({ items }: { items: TeamMomentRow[] }) {
  const router = useRouter()
  const [isPending, startTransition] = useTransition()
  const [pendingId, setPendingId] = useState<string | null>(null)

  async function togglePublished(item: TeamMomentRow) {
    setPendingId(item.id)
    const form = new FormData()
    form.set("published", String(!item.published))
    await fetch(`/api/admin/team/moments/${item.id}`, { method: "PATCH", body: form })
    setPendingId(null)
    startTransition(() => router.refresh())
  }

  async function handleDelete(item: TeamMomentRow) {
    if (!window.confirm(`"${item.caption ?? "Bu yozuv"}" o'chirilsinmi?`)) return
    setPendingId(item.id)
    await fetch(`/api/admin/team/moments/${item.id}`, { method: "DELETE" })
    setPendingId(null)
    startTransition(() => router.refresh())
  }
  ...
  if (items.length === 0) {
    return (
      <p className="rounded-xl border border-white/10 bg-white/[0.02] p-8 text-center text-sm text-slate-400">
        Hali jamoa lahzalari qo&apos;shilmagan.
      </p>
    )
  }
        ...
        <tbody className="divide-y divide-white/5">
          {items.map((item) => (
            <tr key={item.id} className="text-slate-200">
              ...
              <td className="px-4 py-3 text-slate-300">{item.caption ?? "—"}</td>
              ...
              <td className="px-4 py-3">
                <div className="flex justify-end gap-3">
                  <Link
                    href={`/admin/team/moments/${item.id}`}
